...
consumers["baseline with whd eligibility"] = [
    Consumer.consumer_from_dataframe(
        ...
        unit_converter=1_000,
        eligible=bool(ofgem_archetypes_df_whd_eligibility.iloc[row]["whd_eligible"]),
    )
...
scenarios_summary_chart_2["size"] = scenarios_summary_chart_2.apply(
    lambda row: (
        row["WHDEligibleSize"] if row["Eligible for support"] else row["WHDIneligibleSize"]
    ),
    axis=1,
)
...
# Create unique names by modifying name attribute to reflect income decile and add size attribute
for scenario_name in list(scenario_weights.keys()):
    for row, consumer in enumerate(consumers[scenario_name]):
        consumer.name = consumer.archetype + "_" + str(consumer.net_income_decile)
        # Add size attribute from the dataframe
        consumer.size = archetypes_net_income_deciles_full_df.iloc[row]["size"]
...
filename = f"{PROJECT_DIR}/outputs/data/scenarios_data_v2_{date_str}.xlsx"
